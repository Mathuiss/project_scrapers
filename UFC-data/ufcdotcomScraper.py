    ...
    with open('ufcfighters.csv', 'w') as csvfile:
        ...
        for offset in range(0, maxoffset+1, 20):
            ...
            for fighter in fighters:
                try:
                    name = fighter.find(
                        'a', class_='fighter-name').text.strip()
                    record = fighter.findAll(
                        'div', class_='main-txt')[0].text.strip()
                    height = fighter.findAll(
                        'div', class_='sub-txt')[1].text.strip()
                    weight = fighter.findAll(
                        'div', class_='main-txt')[2].text.strip()
                    writer.writerow([name, record.replace('"', ''), re.sub(
                        "[^0-9]", "", height), re.sub("[^0-9]", "", weight)])
                except:
